...
<script>
    ...
    function load_Student_data(data) {
        ...
        for (var i = 0; i < data.length; i++) {
            $('#studentDetailsTable').append('<tr><td>' + data[i].studentID + '</td><td>' + data[i].first_name + ' ' + data[i].last_name + '</td><td>' + data[i].email + '</td><td>' + data[i].phone + '</td><td>' + data[i].address + '</td><td>' + data[i].course_name + '</td><td>' + data[i].year_name + '</td><<td><a href="AddUser.aspx" ><button id=\'myBtn\'>Update</button></a></td>/tr>');
        }
    }
