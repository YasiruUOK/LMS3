<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ApproveFinePayments.aspx.cs" Inherits="LMS.ApproveFinePayments" %>
...
<script>
    ...
    function getFineDetails() {
        ...
        $.ajax({
            type: "GET",
            url: "api/myapi/getNotApprovedPayments",
            async: false,
    ...
    function load_getBookDetails(data) {
        ...
        for (var i = 0; i < data.length; i++) {
            $('#bookDetailsTable').append('<tr><td>' + data[i].bookCode + '</td><td>' + data[i].bookTitle + '</td><td>' + data[i].studentID + '</td><td>' + data[i].first_name + ' ' + data[i].last_name + '</td><td>' + data[i].issueDateString + '</td><td>' + data[i].returnDateString + '</td><td style="text-align:right">' + data[i].fineAmount + '</td><td>' + data[i].TransactionID + '  -  ' + data[i].branchName + '</td><td><input  type=\'button\' onclick=\'approvePayments("' + data[i].paymentID + '","' + data[i].bookCode + '")\' value=\'Approve Payment\' /><input  type=\'button\' onclick=\'rejectPayment("' + data[i].paymentID + '","' + data[i].bookCode +'")\' value=\'Reject Payment\' /></td></tr>');
            totalFineAmount += data[i].fineAmount;
            $("#totalFine").val(totalFineAmount);
        }
    }
    function approvePayments(paymentID, bookCode) {
        if (confirm("Are you sure to Approve Payments? ")) {
            $.ajax({
                type: "GET",
                url: "api/myapi/ApprovePayments",
                contentType: "application/json; charset=utf-8",
                async: false,
    ...
    function deleteFromBookIssueTemp(bookCode) {
        $.ajax({
            type: "GET",
            url: "api/myapi/deleteFromBookIssueTemp",
            contentType: "application/json; charset=utf-8",
            async: false,
    ...
    function rejectPayment(paymentID, bookCode) {
        if (confirm("Are you sure to Reject Payments? ")) {
            $.ajax({
                type: "GET",
                url: "api/myapi/RejectPayments",
                contentType: "application/json; charset=utf-8",
                async: false,
                data: { paymentID: paymentID },
                dataType: "json",
    ...
    function deleteBookIssue(bookCode) {
        $.ajax({
            type: "GET",
            url: "api/myapi/deleteBookIssue",
            contentType: "application/json; charset=utf-8",
            async: false,
            data: { bookCode: bookCode },
            dataType: "json",
    ...
    function copyFromBookIssueTemp(bookCode) {
        $.ajax({
            type: "GET",
            url: "api/myapi/copyFromBookIssueTemp",
            contentType: "application/json; charset=utf-8",
            async: false,
            data: { bookCode: bookCode },
            dataType: "json",
                                    ...
                                    <table class="table table-bordered table-striped tbl_fine_dt" id="bookDetailsTable"
                                        ...
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="fin_notes" style="">Transaction ID and Branch</th>
